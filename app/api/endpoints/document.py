...
async def upload_and_ingest_document(
    ...
    try:
        ...
        result = await asyncio.to_thread(
            ingest_service.ingest, temp_file_path, file.filename
        )
        ...
        logger.error(
            f"Échec critique de l'ingestion pour {file.filename}: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_ingested_chunks(
    ...
    try:
        ...
        logger.error(
            f"Erreur lors du monitoring des métadonnées Qdrant : {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
