...
class RAGService:
    ...
    async def evaluate_performance(
        self, question: str, response: str, retrieved_docs: List[Any]
    ):
        try:
            context_text = self.format_docs(retrieved_docs)
            context_list = [c["content"] for c in retrieved_docs]

            expected_output = await self.generate_reference_answer(
    ...
    def hybrid_search(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        potential_tags = [
            kw for kw in ["diarrhée", "toux", "fièvre", "asthme"] if kw in query.lower()
        ]
        ...
        if potential_tags:
            search_filter = Filter(
                must=[
                    FieldCondition(
                        key="metadata.clinical_tags", match=MatchAny(any=potential_tags)
                    )
                ]
            )
    ...
    async def ask(self, question: str) -> Dict[str, Any]:
        ...
        with mlflow.start_run(run_name="RAG_CLINIQ"):
            ...
            if chunks_list:
                pairs = [
                    [question, c["content"]] for c in chunks_list if c.get("content")
                ]
                if not pairs:
                    sorted_chunks = []
                else:
                    ...
                    sorted_chunks = sorted(
                        chunks_list, key=lambda x: x["rerank_score"], reverse=True
            ...
            metrics = await self.evaluate_performance(
                question, response.text, sorted_chunks
            )
